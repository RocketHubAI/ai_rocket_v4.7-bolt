...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!twilioAccountSid || !twilioAuthToken || !twilioWhatsAppNumber) {
      throw new Error("Missing Twilio WhatsApp configuration. Please set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, and TWILIO_WHATSAPP_NUMBER secrets.");
    }

    const supabase = createClient(supabaseUrl, supabaseServiceKey);

    const { user_id, phone_number, message, event_type, event_id }: WhatsAppRequest = await req.json();
      ...
      if (event_id) {
        await supabase
          ...
          .update({
            ...
            error_message: twilioResult.message || "WhatsApp send failed",
