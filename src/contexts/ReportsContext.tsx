...
export const ReportsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchTemplates = useCallback(async () => {
    ...
    try {
      ...
      // Check what data categories the team has
      const { data: categoryData } = await supabase.rpc('get_team_category_counts', {
        p_team_id: teamId
      });
  ...
  const fetchReportMessages = useCallback(async () => {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .or(`deliver_at.is.null,deliver_at.lte.${now}`)
        .order('created_at', { ascending: false });
  ...
  useEffect(() => {
    if (user) {
      const initializeData = async () => {
        try {
          await Promise.all([
            fetchTemplates().catch(err => console.error('Error fetching templates:', err)),
            fetchUserReports().catch(err => console.error('Error fetching user reports:', err)),
            fetchReportMessages().catch(err => console.error('Error fetching report messages:', err))
          ]);
        } catch (err) {
          console.error('Error initializing reports data:', err);
        }
      };
      initializeData();
