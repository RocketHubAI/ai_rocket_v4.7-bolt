...
export const useGroupChat = () => {
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (user?.id) {
        trackFeatureUsage(user.id, 'team_chat');
      }
  ...
  useEffect(() => {
    ...
    const setupSubscription = async () => {
      ...
      console.log('🔄 Real-time: User team_id:', userTeamId);
    };
    ...
    const channel = supabase
      ...
      .on('postgres_changes', {
        ...
        const senderTeamId = senderData?.team_id;
        console.log('🔄 Real-time: Sender team_id:', senderTeamId, 'User team_id:', userTeamId);

        if (senderTeamId !== userTeamId) {
          console.log('🔄 Real-time: Message from different team, ignoring');
          return;
        }
